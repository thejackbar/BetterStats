/* BetterIQ — shared UI kit (premium elevation)
   Atoms + signature data-viz primitives. Exported to window for cross-file use. */
...
function Icon({ name, size = 18, sw = 1.6, className = '', style }) {
  return (
...
function CountUp({ value, decimals = 0, duration = 1000, prefix = '', suffix = '', className = '', style }) {
  ...
  useEffect(() => {
    ...
    // Fallback: rAF is paused while the document is hidden, which would leave
    // the number stuck at 0. setTimeout fires regardless, so guarantee the
    // final value lands even if the animation never ran.
...
function Sparkline({ values, w = 120, h = 44, stroke = 'var(--pb-accent)', fill = true, strokeWidth = 1.6, dots = false }) {
...
function Bar({ pct, color = 'var(--pb-accent)', h = 8, track = 'var(--pb-surface2)', delay = 0 }) {
  return (
...
/* ── Gauge — win% arc dial ──────────────────────────────────────────────────── */
function Gauge({ value = 0, size = 132, label, sub, color = 'var(--pb-accent)' }) {
...
function ResultPills({ form, size = 22 }) {
  return (
...
function SplitBar({ segments, h = 12 }) {
...
/* ── Card / Stat / Note ──────────────────────────────────────────────────────── */
function Card({ title, eyebrow, right, children, accent = false, className = '', style }) {
  return (
...
function Stat({ label, value, sub, tone, big = false, decimals = 0, suffix = '', count = true }) {
...
/* ── Tag / Btn / Segmented / Search / Empty / Chip ──────────────────────────── */
const TAG_TONE = {
...
function Btn({ children, variant = 'ghost', sm = false, icon, onClick, disabled, className = '', title }) {
  return (
...
function Segmented({ options, value, onChange, sm = false }) {
  return (
...
function Search({ value, onChange, placeholder = 'Search…', className = '', autoFocus }) {
  return (
...
/* surname helper */
function surname(name) { const s = (name || '').split(',')[0].trim(); return s || name || '?'; }
...
function Delta({ value, suffix = '', decimals = 0, neutralBand = 0 }) {
...
/* ── Initials chip ───────────────────────────────────────────────────────── */
function Initials({ name, size = 36, tone }) {
...
Object.assign(window, {
  Icon, CountUp, Sparkline, Bar, Gauge, ResultPills, SplitBar, StackedBar, Heatmap,
  Card, Stat, Note, Tag, Btn, Segmented, Search, Empty, surname,
  Tabs, PageIntro, Delta, Initials, KV,
});
