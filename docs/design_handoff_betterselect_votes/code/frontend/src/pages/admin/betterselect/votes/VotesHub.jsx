...
export default function VotesHub({ canManage, onOpenFixture }) {
  ...
  const bulk = async (action) => {
    ...
    try {
      ...
      toast.success(`${r.updated} game${r.updated === 1 ? '' : 's'} ${action === 'open' ? 'opened' : 'locked'}`)
      clear(); load()
  ...
  const focus = rows.find((r) => r.state === 'open') || null
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col lg:flex-row gap-5">
        ...
        <div className="w-full lg:w-[300px] shrink-0">
          <OutstandingVoters fixture={focus} onNudged={load} />
        </div>
      </div>
    </div>
  )
