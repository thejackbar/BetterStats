...
- `iq_selection.py` (`/iq/selection/*`) analyses a fixture's saved BetterSelect lineup (`fixture_lineups`): XI **balance** (pace/spin, keeper, openers, all-rounders, LH/RH — from `players.skill_positions` + `bowling_type`/`bowling_action`), each pick's last-5 **form**, **warnings** (no keeper, thin attack `<5` bowlers, picked-but-`UNAVAILABLE`, out-of-form bat `<15` avg), **promote** (in-form active players left out) / **rest** (out-of-form picks), and playing up/down via `teams.sequence`. Availability = explicit `player_availability` for the match date (no period resolution yet).
