...
- **Phase 2b (deferred, needs testing)** — teach `sync_grassroots_game_level_data` to translate scorecard `participantId` (raw GUID) → per-club `uuid5` id so a per-club player gets game-level (per-innings) rows too. Until then per-club players are **aggregate-only** (career totals correct, per-game breakdown empty). Left out of this pass because it's the hot path that runs for every club; it must be verified on a data copy (legacy orgs must stay byte-identical — the translation is the identity map for them — and the shared player must get his game-level rows).
