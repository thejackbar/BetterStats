...
- **Email sending is the one remaining DNS dependency.** SPF/DKIM/DMARC are configured on `betterstats.cricket`, so the From (`config/settings.py` `email_from_address`) **stays `noreply@betterstats.cricket` for now** (the authenticated domain). The from-name is already "Better Cricket" and the default reply-to is `betteratcricket@gmail.com` (a monitored inbox; a `@gmail.com` address must NOT be the From, it fails Gmail's DMARC). Once SPF/DKIM/DMARC exist on `betterat.cricket`, flip the From to `noreply@betterat.cricket`. `email_provider` defaults to `console`, so nothing sends until a provider is configured anyway.
