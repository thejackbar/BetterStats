...
The canonical public domain is **`https://betterat.cricket`** (no `www`), the "Better Cricket" brand. The old `betterstats.cricket` now **permanently redirects to `betterat.cricket`** (301 for GET/HEAD, 308 otherwise) via `cloudflare-worker/worker.js`, so the old domain's link equity consolidates onto the canonical and existing links keep working. The older `betterstats.bltbox.com` domain is retired.
...
- **Email sending is the one remaining DNS dependency.** SPF/DKIM/DMARC are currently configured on `betterstats.cricket`. The From (`config/settings.py` `email_from_address`) is now `noreply@betterat.cricket`, the from-name is "Better Cricket" and the default reply-to is `betteratcricket@gmail.com` (a monitored inbox; a `@gmail.com` address must NOT be the From, it fails Gmail's DMARC). **Set up SPF/DKIM/DMARC on `betterat.cricket` before sending live mail**, or pin `EMAIL_FROM_ADDRESS=noreply@betterstats.cricket` in the server `.env` until those records exist (`email_provider` defaults to `console`, so nothing sends until a provider is configured anyway).
...
- `cloudflare-worker/worker.js` is now a pure old-domain redirect (its old OG-injection job is handled by `og_preview`). Deploy with `wrangler deploy` and keep the Cloudflare route `betterstats.cricket/*` active.
