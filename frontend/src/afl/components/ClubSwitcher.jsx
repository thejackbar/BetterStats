...
// Super-admin (Better HQ staff) club switcher — lets staff re-scope the whole
// AFL admin app to any club without logging out. Mirrors cricket's
// components/admin/ClubSwitcher.jsx exactly (same backend: routers/auth.py's
// /auth/switch-club + active_club_id are shared, sport-agnostic code already
// mounted in afl_main.py) — this is an AFL-native port only because cricket's
// version calls lib/api.js (the cricket API client) and expects a bare-array
// response, where aflApi's superListClubs returns { clubs, total }.
export default function ClubSwitcher() {
  ...
  useEffect(() => {
    ...
    aflApi.superListClubs()
      .then(res => setClubs(Array.isArray(res?.clubs) ? res.clubs : []))
      .catch(() => setClubs([]))
  }, [open, clubs])
