      ...
      <div className="max-w-[1200px] mx-auto p-4 sm:p-6">
        <div className="mb-5">
          ...
          <p className="text-sm text-pb-dim mt-1">
            The national cricket-club list crawled from the Cricket Australia grassroots
            directory, for BetterCricket outreach. One contact per club (the published club
            contact, usually a role mailbox); office bearers are not in the source data.
          </p>
        </div>
                  ...
                  return (
                    <tr key={c.id} className="border-t pb-hairline align-top">
                      <td className="px-3 py-2">
                        <div className="font-medium text-pb-text flex items-center gap-1.5">
                          {c.name}
                          {c.is_customer && <span className="text-[10px] text-emerald-300 border border-emerald-500/40 rounded px-1">customer</span>}
                        </div>
                        {c.website_url && (
                          <a href={c.website_url} target="_blank" rel="noreferrer" className="text-[11px] text-pb-dim hover:text-pb-accent">
