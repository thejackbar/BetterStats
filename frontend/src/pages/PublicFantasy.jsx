...
import { FantasyCtx, crestText, DISP } from './fantasy/ui'
...
export default function PublicFantasy() {
  ...
  const screen = () => {
    switch (view) {
      case 'team': return <MyTeam token={token} manager={manager} squad={squad} season={season} round={round} onChange={loadApp} flash={flash} fail={fail} nav={nav} />
      ...
      case 'transfers': return <Transfers token={token} squad={squad} pool={pool} rules={rules} round={round} onChange={loadApp} flash={flash} fail={fail} nav={nav} />
      case 'chips': return <Chips token={token} squad={squad} round={round} onChange={loadApp} flash={flash} fail={fail} nav={nav} />
      case 'points': return <Points token={token} nav={nav} />
  ...
  return (
    <FantasyCtx.Provider value={ctx}>
      <div className="bfc-root" data-theme={theme} style={rootSx}>
        <div style={{ maxWidth: 540, margin: '0 auto', padding: '18px 16px 56px' }}>
          ...
          {phase === 'auth' && <Auth token={token} season={season} onAuthed={onAuthed} />}

          {phase === 'app' && (
            <>
              <AppHeader season={season} manager={manager} unread={unread} onBell={() => nav('notifications')} onSearch={() => nav('stats')} onProfile={() => nav('settings')} />
              <NavPills items={NAV.map(([k, l]) => [k, k === 'pick' && squad ? 'Edit' : l])} value={view} onChange={(v) => nav(v)} />
              <div style={{ display: 'flex', gap: 6, padding: '2px 0 10px', overflowX: 'auto', scrollbarWidth: 'none' }}>
                {QUICK.map(([k, l]) => {
                  const on = view === k
                  return (
