      ...
      <table className="w-full text-left">
        <thead>
          <tr className="text-pb-faint font-mono text-[10px] tracking-wide3 bg-pb-surface2/40">
            ...
            <th className="font-medium py-2.5 px-3">INN</th>
            <th className="font-medium py-2.5 px-3 cursor-pointer hover:text-pb-text select-none" onClick={() => toggle('runs')}>
              R<SortIcon colKey="runs" />
            </th>
            <th className="font-medium py-2.5 px-3 cursor-pointer hover:text-pb-text select-none" onClick={() => toggle('balls')}>
              B<SortIcon colKey="balls" />
            </th>
            <th className="font-medium py-2.5 px-3">4s</th>
            <th className="font-medium py-2.5 px-3">6s</th>
            <th className="font-medium py-2.5 px-3 cursor-pointer hover:text-pb-text select-none" onClick={() => toggle('strike_rate')}>
              SR<SortIcon colKey="strike_rate" />
            </th>
            <th className="font-medium py-2.5 px-3">HO</th>
          </tr>
        </thead>
        <tbody>
          {sorted.map((row, i) => {
            ...
            return (
              <tr key={i} className={`${i ? 'pb-hairline-t' : ''} hover:bg-pb-surface2`}>
                ...
                <td className="py-2.5 px-3 font-mono text-[11px] text-pb-faint">{row.innings_number ?? '—'}</td>
