...
//
// Props:
//   types         [{ key, label, count }]   one per post type
...
//   onOpenEditor()             open the editor on the last/default type
...
function TypeCard({ type, onClick }) {
  return (
    <button onClick={onClick}
      ...
      <span className="block h-[74px] rounded-md mb-[7px] grid place-items-center"
        style={{ background: 'linear-gradient(135deg, color-mix(in srgb, var(--pb-accent) 22%, #10141d), #10141d)' }}>
        <span className="font-mono text-[10px] tracking-wide2 uppercase text-pb-dim">{type.label}</span>
      </span>
...
export default function StartScreen({
  types = [], club = {}, moduleLogo, savedTemplates = [],
  onPick, onOpenEditor, onApplyTemplate,
}) {
  return (
    <div className="fixed inset-0 z-40 flex flex-col items-center justify-center gap-[26px] px-6 md:px-[60px] py-10 bg-pb-bg text-pb-text overflow-auto">
      ...
      <div className="w-full max-w-[1120px] pt-5 border-t pb-hairline flex items-center justify-between gap-4 flex-wrap">
        ...
        <button onClick={onOpenEditor}
          ...
          Open the editor <Icon name="arrow" size={13} />
        </button>
      </div>
    </div>
  )
