...
// BetterIQ's own focused nav. Phase 1 ships Opposition analysis; selection
...
export default function IQLayout({ children, title, actions }) {
    ...
    <div className="px-4 py-4 border-b pb-hairline">
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0">
          ...
          <div className="font-mono text-[10px] text-pb-faint">Better<span style={{ color: 'var(--pb-accent)' }}>IQ</span></div>
        </div>
      </div>
  ...
  return (
    // BetterIQ is BetterStats-branded chrome (not the club's white-label site).
    // Re-point --pb-accent to a fixed violet for the whole module — the "brain"
    // accent — so it reads distinctly from BetterSelect's green and never picks
    // up the club's (possibly alarming-red) public accent. Custom properties
    // inherit, so this one override cascades to every child.
    <div className="min-h-screen bg-pb-bg text-pb-text flex"
      style={{ '--pb-accent': '#8b7cf6', '--pb-accent-rgb': '139 124 246' }}>
      {/* Sidebar */}
