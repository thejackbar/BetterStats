...
export function Field({ label, children, hint, half, width, composite }) {
  ...
  //
  // `composite` renders a plain grouped div instead of the <label>. A <label>
  // forwards a click on ANY descendant to whichever labelable control it
  // holds at that moment, so a field wrapping its own buttons (a checkbox
  // picker, a chip multi-select) can cancel its own selection the instant a
  // click lands on a re-rendered control — the trap the Accounts person
  // picker hit, and the reason MultiSelect below is always given this.
  const style = width ? { flexBasis: width, maxWidth: width, minWidth: 0 } : undefined
  const cls = half && !width ? 'block flex-1 min-w-0 basis-[calc(50%-6px)]' : width ? 'block flex-none' : 'block'
  const inner = (
    <>
  ...
  if (composite) return <div className={cls} style={style} role="group" aria-label={label}>{inner}</div>
  return <label className={cls} style={style}>{inner}</label>
}
