export default {
  ...
  items: [
    "The CLEAN OPPOSITION STATS admin button (added in v7.18.5) was wiping legacy players whose stats only exist as career summaries (e.g. older aggregate totals, no scorecards, no appearances). Symptom: player count on the club dashboard suddenly dropped and individual profiles for those players reset to zero: reported for Applecross (1573 players in DB, only 1516 with stats showing).",
    ...
    "If you clicked CLEAN OPPOSITION STATS before this deploy, run Sync Now to restore any legit data that the aggregate API still surfaces. Players whose data has been retired from the per-season API are permanently lost on our side and would need to be re-entered via Manual Season Adjustments.",
  ],
}
