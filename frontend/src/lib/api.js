...
export const api = {
  ...
  classifyGrade: (gradeName, { category, categories, match_formats, is_public, display_order } = {}) =>
    request('/admin/grades/classify', {
      method: 'PATCH',
      // Only send what the caller actually set: the endpoint reads an omitted
      // field as "leave unchanged" and an empty array as "clear this back to
      // the suggestion", so a JSON `null` for an untouched axis would wipe it.
      body: JSON.stringify({
        grade_name: gradeName,
        ...(category !== undefined ? { category } : {}),
        ...(categories !== undefined ? { categories } : {}),
        ...(match_formats !== undefined ? { match_formats } : {}),
  ...
  crmPinActivity: (dealId, activityId, pinned) => request(`/club-admin/crm/deals/${dealId}/activities/${activityId}`, { method: 'PATCH', body: JSON.stringify({ pinned }) }),
  ...
  superCrmPinActivity: (dealId, activityId, pinned) => request(`/club-admin/super/crm/deals/${dealId}/activities/${activityId}`, { method: 'PATCH', body: JSON.stringify({ pinned }) }),
  ...
  salesWorkspaceEventOwners: () => request('/club-admin/sales-workspace/event-owners'),
  ...
  salesWorkspaceBulkAssign: (dealIds, ownerUserIds, unassign = false, confirmReassign = false) =>
    request('/club-admin/sales-workspace/bulk-assign', {
      method: 'POST',
      body: JSON.stringify({
        deal_ids: dealIds, owner_user_ids: ownerUserIds, unassign, confirm_reassign: confirmReassign,
      }),
    }),
