...
// ─── The Super Admin modal ───────────────────────────────────────────────────
{
  ...
  await modal.locator('input[placeholder="Discount code (optional)"]').fill('TENOFF').catch(() => {})
  await modal.getByRole('button', { name: 'APPLY', exact: true }).click().catch(() => {})
  await page.waitForTimeout(700)
  ck('a discount code prices into the live preview', /Ten percent \(TENOFF\)/.test(await text(byTestId(page, 'invoice-quote'))), await text(byTestId(page, 'invoice-quote')))
  ...
  ck('the discount code goes on the wire with the invoice', req?.payload?.coupon_code === 'TENOFF', JSON.stringify(req?.payload))
