...
async def _send_code_email(email: str, code: str) -> None:
    ...
    msg = email_service.EmailMessage(
        ...
        html=html,
        text=text,
        # settings.email_from_address is the platform's dedicated system-
        # notifications identity (notifications@bettersports.com.au), verified
        # in SES for exactly this kind of platform-level send — see
        # config/settings.py. reply_to stays the human-monitored support inbox.
