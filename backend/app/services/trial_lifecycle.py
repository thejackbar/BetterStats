...
A daily scan that turns a club's own trial milestones into real outbound
email to its primary admin. A trial club has nobody watching a CRM — these are
the reminders that actually reach them — so it is deliberately CRM-independent
and covers every club with a module trial.

Six nudge types, each checked against ``trial_lifecycle_nudges.dedupe_key``
before sending and recorded only after a successful send, so the daily scan
never re-sends the same nudge twice and a provider failure or crash mid-send
gets retried on the next scan instead of being silently marked done:
