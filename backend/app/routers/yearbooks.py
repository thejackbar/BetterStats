...
async def get_yearbook(org_id: str, season_id: str, db: AsyncSession = Depends(get_db)):
    ...
    featured = await db.execute(
        text("SELECT achievement_id FROM yearbook_featured_achievements WHERE yearbook_id = CAST(:yid AS uuid) ORDER BY sort_order, id"),
        {"yid": str(yb["id"])},
    )
...
async def add_featured_achievement(
    ...
    await db.execute(
        text("""
            ...
            VALUES (CAST(:yid AS uuid), :aid, :order)
...
async def remove_featured_achievement(
    ...
    yb = await _ensure_stub(db, org_id, season_id)
    await db.execute(
        text("DELETE FROM yearbook_featured_achievements WHERE yearbook_id = CAST(:yid AS uuid) AND achievement_id = :aid"),
