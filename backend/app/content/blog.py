...
BLOG_POSTS: list[dict] = [
    {
        ...
        "body": [
            ...
            {"type": "p", "text": "The obvious fix is to read the competition off the fixture data and group by it. We checked before building anything, and it is not there. Cricket Australia's feed gives us seasons, grades, matches and full scorecards, and at no point does it name the competition a grade sits under. PlayHQ's own website shows it, but that sits behind an interface we cannot rely on for something a club presses a button on."},
            {"type": "p", "text": "What the feed does carry, on every single grade, is the association that runs it. That turned out to be sitting in a response we were already fetching and throwing away, and it goes back as far as a club's records do. We checked against a club whose records run back to the mid-1970s, and the association was on its oldest season."},
        ...
        "faq": [
            ...
            {"q": "Does PlayHQ tell BetterCricket which competition a grade belongs to?", "a": "No. The fixture feed carries seasons, grades, matches and scorecards, but never names the competition. It does name the association that runs each grade, so BetterCricket groups on that automatically and lets a club split it further where one association runs several competitions."},
