...
BLOG_POSTS: list[dict] = [
    {
        ...
        "body": [
            ...
            {"type": "h2", "text": "The fixture data does not have a competition in it"},
            {"type": "p", "text": "The obvious fix is to read the competition off the fixture data and group by it. We checked before building anything, and it is not there. The data behind a club's fixtures covers seasons, grades, matches and full scorecards, and at no point names the competition a grade sits under. It is written down elsewhere, but nowhere we could read reliably enough to hang a button on."},
            {"type": "p", "text": "What the fixture data does carry, on every single grade, is the association that runs it. That turned out to be sitting in something we already had and were not using, and it goes back as far as a club's records do. We checked against a club whose records run back to the mid-1970s, and the association was on its oldest season."},
        ...
        "faq": [
            ...
            {"q": "Does the fixture data say which competition a grade belongs to?", "a": "No. It carries seasons, grades, matches and scorecards, but never names the competition. It does name the association that runs each grade, so BetterCricket groups on that automatically and lets a club split it further where one association runs several competitions."},
    ...
    },
    {
        ...
        "faq": [
            ...
            {"q": "Can BetterCricket import a spreadsheet with dozens of tabs going back a hundred years?", "a": "Yes. Clubs bring in whatever they have, however it is organised, and BetterCricket reconciles it against the club's own synced match data into one career history per player. The historical import is included in the subscription, not charged as a separate fee."},
