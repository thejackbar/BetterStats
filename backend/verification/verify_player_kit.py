...
What is asserted:
  * migration 289 applied three times to a populated pre-289 schema, and the
...
# Every column migration 289 adds, and the table it belongs to.
...
def migration_statements() -> list[str]:
    ...
    src = (Path(__file__).resolve().parent.parent / "alembic" / "versions" / "289_player_kit.py").read_text()
...
async def strip_to_pre289() -> None:
...
async def main() -> None:
    print("\n-- migration 289, applied to a populated pre-289 schema --")
    await build_schema()
    async with Session() as session:
        await seed(session)
        await session.commit()
    await strip_to_pre289()
    async with Session() as session:
    ...
    async with Session() as session:
        ...
        check("every pre-289 row reads as no number and no size recorded",
